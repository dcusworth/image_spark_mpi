...
cpdef train(np.ndarray[np.float64_t, ndim=2] train_data, 
	      np.ndarray[np.float64_t, ndim=2] denom_noninv,
		  np.ndarray[np.int64_t, ndim=1] train_labels, 
		  float ll,
		  int num_threads):
	"""
	Train regression on image data using cython using Data Parallelism.
	...
	Returns
	-------
	:type iws: numpy.ndarray (of numpy.float64)
	:param iws: Weights for each digit 
	
	Written by Tim Clements, Dan Cusworth, and Bram Maasakkers
	Last modified 5/1/17

	"""
	cdef int choose_label, q, i, j, idx, p, N = 10
	cdef int Nt = train_data.shape[0],Mt = train_data.shape[1]
	cdef float idenom, vacc, tp, topenmp, start, end 
	...
	cdef np.ndarray[np.float64_t, ndim=2] iws = np.empty([N,Mt],dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=2] denom_noninv = np.empty([Mt,Mt],dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=2] denom_sum = np.empty([Mt,Mt],dtype=DTYPE64)

	# Add regularization parameter
	denom_noninv = denom_noninv + np.eye(Mt)*ll
	...
	for choose_label in range(N): 
		...
		for j in prange(Mt,nogil=True,schedule='static',num_threads=num_threads):
			for i in range(Nt):
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef test(np.ndarray[np.float64_t, ndim=2] test_data, 
		 np.ndarray[np.int64_t, ndim=1] test_labels, 
		 np.ndarray[np.float64_t, ndim=2] iws, 
		 int num_threads):
	"""
	Test regression on image data using cython using Data Parallelism. 
	...
	cdef float denom_sum, idenom, vacc
	cdef np.int64_t imost
	cdef np.ndarray[np.float64_t, ndim=1] numer_sum = np.empty(Mtest,dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=1] iw = np.empty_like(numer_sum)
	cdef np.ndarray[np.float64_t, ndim=1] iout = np.empty(Ntest,dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=1] iclass = np.empty_like(iout)
	cdef np.ndarray[np.float64_t, ndim=2] ws = np.empty([N,Mtest], dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=2] iouts = np.empty([N,Ntest],
														 dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=2] iclasses = np.empty([N,Ntest],
														 dtype=DTYPE64)
	cdef np.ndarray[np.float64_t, ndim=1] ipred = np.empty_like(iout)
	cdef np.ndarray[np.float64_t, ndim=1] preds = np.empty(Ntest,dtype=DTYPE64)
	...
	for choose_label in range(N):
		...
		for i in prange(Ntest,nogil=True,schedule='static',num_threads=num_threads):
			for j in range(Mtest):
				iout[i] += test_data[i,j] * iws[choose_label,j] 
...
cpdef matmat_multi_tile(np.ndarray[np.float64_t, ndim=2] A, 
	...
	"""
	cdef int n = A.shape[0],m = A.shape[1], b = 98, b_two = 100
